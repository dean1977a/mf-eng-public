...
def missing_column_values_count(df):
...
# Function to calculate missing values by row# Funct
def missing_row_values_count(df):
    # Total missing values
    mis_val = df.isnull().sum(axis=1)

    # Percentage of missing values
    mis_val_percent = 100 * df.isnull().sum(axis=1) / len(df.columns)
    ...
    # Sort the table by percentage of missing descending
    mis_val_table_ren_rows = mis_val_table_rows[
    ...
    print ("Your selected dataframe has " + str(df.shape[1]) + " columns.\n"
                                                               "There are " + str(
        mis_val_table_ren_rows.shape[0]) +
           " columns that have missing values.")
...
categorical_var, dis_num_list, numerical_var = feature_type_split(trainData, special_num_list=['回溯_近7日APP使用次数', ],
                                                                  special_cat_list=['HAS_CHILD', 'BJ_HIT_RULE',
                                                                                    'BJ_BLACK_LIST',
                                                                                    'BAIDU_BLACK_LIST'])
